...
```bash
# Bootstrap a new agent + local context + token storage
...
Active context switching rules:

- if no currently authenticated active context exists, new context becomes active automatically
- if an authenticated active context already exists, register keeps it active
- pass `--log-in` to force switching active context to the newly registered agent
...
- Create/update and activate a context:
...
- Switch to an existing context:
  ...
  - if multiple contexts match the same agent, provide additional identifying info like `--base-url`
- Inspect remote agents plus local contexts:
  - `cafaye agents list`
