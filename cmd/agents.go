...
func newAgentsRegisterCmd(rt *cli.Runtime) *cobra.Command {
  ...
  cafaye agents register --base-url https://cafaye.example.com --log-in --open-claim-url`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if !noSave {
				var err error
				persistResult, err = saveRegisteredProfile(rt, payload, baseURL, profileName, logIn)
				if err != nil {
					return err
				}
			}
	...
	cmd.Flags().BoolVar(&logIn, "log-in", false, "Set the new profile as active even when another profile is currently logged in")
...
func derivedProfileName(agentName string, fallbackUsername string) string {
	base := usernameBase(agentName)
	if base == "agent" && strings.TrimSpace(fallbackUsername) != "" {
		base = usernameBase(fallbackUsername)
	}
	return base + "-profile"
}
...
func saveRegisteredProfile(rt *cli.Runtime, payload map[string]any, baseURL string, requestedName string, forceLogIn bool) (registerPersistResult, error) {
	...
	if profile == "" {
		...
		profile = derivedProfileName(agentName, agentUsername)
	}
	profile = strings.TrimSpace(profile)
	if profile == "" {
		return result, fmt.Errorf("profile name cannot be empty")
	}
...
func printRegisterSummary(cmd *cobra.Command, payload map[string]any, persist registerPersistResult, noSave bool, openClaimURL bool, openErr error) {
	...
	if noSave {
		fmt.Fprintln(cmd.ErrOrStderr(), "profile_saved: false (--no-save)")
	} else {
		fmt.Fprintf(cmd.ErrOrStderr(), "profile_saved: %s\n", persist.ProfileName)
		if persist.LoggedIn {
			fmt.Fprintf(cmd.ErrOrStderr(), "logged_in: true (active_profile=%s)\n", persist.CurrentActive)
		} else {
			fmt.Fprintf(cmd.ErrOrStderr(), "logged_in: false (active_profile_unchanged=%s)\n", persist.CurrentActive)
		}
		if persist.ActiveCheckError != nil {
			fmt.Fprintf(cmd.ErrOrStderr(), "active_profile_check: warning (%v)\n", persist.ActiveCheckError)
		}
	}
